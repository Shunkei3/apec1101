...
$$
\left. MRS \text{ for good X for good Y} = \frac{\Delta Y}{\Delta X} \right|_{U=\bar{u}} = \left| \frac{MU_X}{MU_Y} \right|
$$
...
::: {.callout-tip} 
## Recall that
+ The marginal rate of substitution of Chicken Sandwiches for French Fries means how many French Fries the consumer is willing to give up in exchange for one more Chicken Sandwiches while maintaining the same level of utility.

+ Recall that the MRS equals the same as the slope of the budget constraint at the utility-maximizing combination.
:::

From part (1), we found that the utility-maximizing combination is (CS, FF) = (1, 6) is 
